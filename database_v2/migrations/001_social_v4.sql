-- ════════════════════════════════════════════════════════════════════════════
-- MIGRACIÓN 001 — Capa social/v4 (notificaciones, amistades, logros, etc.)
-- ════════════════════════════════════════════════════════════════════════════
-- Ejecutado AUTOMÁTICAMENTE por database_v2/migrate.js al iniciar la app
-- (de forma segura/idempotente: CREATE TABLE IF NOT EXISTS, ADD COLUMN IF
-- NOT EXISTS, INSERT IGNORE). Soluciona el error:
--   {"error":"Table 'tienda_ropa.notificaciones' doesn't exist"}
-- y desbloquea TODAS las tablas que dependen de esta migración:
--   notificaciones, amistades, logros, usuario_logros, perfil_conexiones,
--   mensajes_chat, encuestas, encuesta_opciones, encuesta_votos,
--   stickers, comentario_stickers, tablero_guardados, tablero_reacciones.
-- Requiere que database_v2/migrations/000_schema_base.sql (o database.sql)
-- ya haya sido importado al menos una vez (crea usuarios, tableros,
-- perfil_config, comentarios, etc. sobre las que ésta se apoya).
-- ════════════════════════════════════════════════════════════════════════════
