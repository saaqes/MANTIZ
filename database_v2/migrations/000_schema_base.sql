-- ════════════════════════════════════════════════════════════════════════════
-- MIGRACIÓN 000 — Esquema base (referencia / ejecución MANUAL)
-- ════════════════════════════════════════════════════════════════════════════
-- Copia de referencia del esquema base original (database.sql), incluido
-- aquí por completitud de database_v2/migrations.
--
-- ⚠️ NO se ejecuta automáticamente con database_v2/migrate.js: usa
-- `DELIMITER $$` / `CREATE PROCEDURE` para sus migraciones de columnas, una
-- sintaxis de CLIENTE (mysql CLI) que el driver mysql2 no soporta igual.
--
-- Para una instalación nueva, impórtalo manualmente UNA vez:
--   mysql -u <user> -p <db> < database_v2/migrations/000_schema_base.sql
-- (o el database.sql original en la raíz del proyecto — son idénticos).
-- Después de esto, database_v2/migrate.js se encarga del resto de forma
-- automática (empezando por 001_social_v4.sql).
-- ════════════════════════════════════════════════════════════════════════════
