...
router.get('/productos/nuevo', isAdmin, (req, res) => {
  res.render('admin/productos/form', { title: 'Nuevo Producto', producto: null, tallas: [], imagenes: [], isAdmin: true });
});
...
router.get('/productos/:id/editar', isAdmin, async (req, res) => {
  ...
  const [imagenes] = await db.query('SELECT * FROM producto_imagenes WHERE producto_id=? ORDER BY orden ASC', [req.params.id]);
  if (!producto) return res.redirect('/admin/productos');
  res.render('admin/productos/form', { title: 'Editar Producto', producto, tallas, imagenes: imagenes || [], isAdmin: true });
});
...
router.post('/productos/:id/editar', isAdmin, uploadProduct.fields([
  ...
  try {
    ...
    await db.query(`UPDATE productos SET ${setClause} WHERE id=?`, [...Object.values(updates), prodId]);

    if (req.files?.imagenes_extra?.length) {
      const [[maxOrden]] = await db.query('SELECT COALESCE(MAX(orden),0) AS m FROM producto_imagenes WHERE producto_id=?', [prodId]);
      let orden = maxOrden.m || 0;
      for (let i = 0; i < req.files.imagenes_extra.length; i++) {
        orden++;
        await db.query('INSERT INTO producto_imagenes (producto_id, imagen, orden) VALUES (?,?,?)', [prodId, req.files.imagenes_extra[i].filename, orden]);
      }
    }

    await db.query('DELETE FROM producto_tallas WHERE producto_id=?', [prodId]);
    const tallasList = Array.isArray(tallas) ? tallas : (tallas ? [tallas] : []);
    const stocksList = Array.isArray(stocks) ? stocks : (stocks ? [stocks] : []);
    for (let i = 0; i < tallasList.length; i++) {
      if (tallasList[i]) {
        await db.query('INSERT INTO producto_tallas (producto_id, talla, stock) VALUES (?,?,?)', [prodId, tallasList[i], stocksList[i] || 0]);
      }
    }
    ...
    req.flash('error', 'Error: ' + e.message);
    res.redirect(`/admin/productos/${prodId}/editar`);
  }
});

router.post('/productos/:id/imagenes/:imgId/eliminar', isAdmin, async (req, res) => {
  try {
    await db.query('DELETE FROM producto_imagenes WHERE id=? AND producto_id=?', [req.params.imgId, req.params.id]);
    req.flash('success', 'Imagen extra eliminada');
  } catch(e) {
    req.flash('error', 'Error al eliminar imagen: ' + e.message);
  }
...
// CARRUSEL MARCA (banner secundario — antes del footer)
router.post('/carrusel-marca/nuevo', isAdmin, (req, res, next) => {
  uploadCarousel.single('imagen')(req, res, (err) => {
    if (err) {
      req.flash('error', err.code === 'LIMIT_FILE_SIZE' ? 'Imagen demasiado grande (máx 20MB)' : 'Error al subir: ' + err.message);
      return res.redirect('/admin/carrusel');
    }
    next();
  });
  ...
  try {
    const { titulo, subtitulo, boton_texto, boton_url, color_acento, activo } = req.body;
    const [[max]] = await db.query('SELECT COALESCE(MAX(orden),0) AS m FROM carrusel_marca');
    const orden = (max.m || 0) + 1;
    const activoVal = activo ? 1 : 0;
    try {
      await db.query(
        'INSERT INTO carrusel_marca (imagen, titulo, subtitulo, orden, activo, boton_texto, boton_url, color_acento) VALUES (?,?,?,?,?,?,?,?)',
        [req.file.filename, titulo || '', subtitulo || '', orden, activoVal,
         boton_texto || null, boton_url || '/productos', color_acento || '#e91e8c']
      );
    } catch (e2) {
      await db.query(
        'INSERT INTO carrusel_marca (imagen, titulo, subtitulo, orden, activo) VALUES (?,?,?,?,?)',
        [req.file.filename, titulo || '', subtitulo || '', orden, activoVal]
      );
    }
    req.flash('success', 'Imagen del carrusel secundario agregada');
  } catch(e) {
    req.flash('error', 'Error al guardar: ' + e.message);
  }
  res.redirect('/admin/carrusel');
...
router.post('/carrusel-marca/:id/editar', isAdmin, (req, res, next) => {
  uploadCarousel.single('imagen')(req, res, (err) => {
    if (err) {
      req.flash('error', err.code === 'LIMIT_FILE_SIZE' ? 'Imagen demasiado grande (máx 20MB)' : 'Error al subir: ' + err.message);
      return res.redirect('/admin/carrusel');
  ...
  try {
    ...
    const updates = {
      titulo: titulo || '', subtitulo: subtitulo || '', activo: (activo !== undefined && activo !== '') ? 1 : 0,
    ...
    try {
      ...
      const basic = { titulo: updates.titulo, subtitulo: updates.subtitulo, activo: updates.activo };
      if (req.file) basic.imagen = req.file.filename;
      const bc = Object.keys(basic).map(k => `${k}=?`).join(',');
      await db.query(`UPDATE carrusel_marca SET ${bc} WHERE id=?`, [...Object.values(basic), req.params.id]);
    }
    req.flash('success', 'Imagen del carrusel secundario actualizada');
  } catch(e) {
